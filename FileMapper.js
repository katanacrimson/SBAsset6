...
module.exports = class FileMapper {
  ...
  async getFile (virtualPath) {
    ...
    switch (options.type) {
      ...
      case 'path':
        ...
        const filelength = options.filelength || (position - size)

        const { buffer } = await fs.read(fd, Buffer.alloc(filelength), 0, filelength, position)
  ...
  async setFile (virtualPath, options) {
    ...
    if (options.pak !== undefined) {
      ...
      throw new TypeError('FileMapper.setFile requires either a SBAsset6 instance, a file descriptor, a filepath, or a Buffer be specified for the origin')
    }
