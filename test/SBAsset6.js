...
describe('SBAsset6 integration test', () => {
  ...
  it.only('should be able to write an SBAsset6 archive', async () => {
    const filename = path.join(__dirname, '/tmp/test.pak')
    const pak = new SBAsset6(filename)
    pak.metadata = {
      priority: 9999999999
    }
  ...
  it('should be able to read the SBAsset6 archive it made', async () => {
    const filename = path.join(__dirname, '/tmp/test.pak.tmp')
    const pak = new SBAsset6(filename)

    const res = await pak.load()
    const fileData = await pak.files.getFile('/universe_server.config.patch')
    console.dir(res)
